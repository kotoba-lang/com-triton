(ns triton.main
  "Kotodama WASM entrypoint for the Triton clean-room actor (L4) — Clojure port.
  ...
  facts (`triton.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "triton")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"triton.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "triton-compat" :tier tier :entities entities} 200])
...
(defn start! [] :triton-compat/ready)
